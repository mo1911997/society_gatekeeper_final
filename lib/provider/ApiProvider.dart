...
class ApiProvider {
  ...
  Future<String> addParcelDetails(flatNumber, userName, image,
      deliveryAgencyName, deliveryAgentContact) async {
        print(flatNumber.toString()+userName.toString()+image.toString()+deliveryAgencyName.toString()+deliveryAgentContact.toString());
    var request =
        http.MultipartRequest("POST", Uri.parse(Constants.BASE_URL + 'parcels/add_parcel'));
    //add text fields
    var storage = FlutterSecureStorage();
    var token = await storage.read(key: 'token');
  ...
  Future<String> getParcelDetails() async {
    var storage = FlutterSecureStorage();
    var token = await storage.read(key: 'token');
    http.Response response = await http.get(Constants.BASE_URL + '',
        headers: {'Authorization': 'Bearer ' + token});
